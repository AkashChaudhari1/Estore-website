...
public class HomeController {
	...
	public String index(Model m) {
		List<Category> allActiveCategory = categoryService.getAllActiveCategory().stream().limit(6).sorted((c1,c2)->c2.getId().compareTo(c1.getId())).toList();
		List<Product> allActiveProducts = productService.getAllActiveProducts("").stream().limit(8).sorted((p1,p2)->p2.getId().compareTo(p1.getId())).toList();
...
//		m.addAttribute("products", products);
	...
	public String saveUser(@ModelAttribute UserDtls user, @RequestParam("img") MultipartFile file, HttpSession session)
		...
		if(existEmail) {
			session.setAttribute("ErrorMsg", "An account with this email already exists.");
		}else {
	...
	public String showForgetPassword(@RequestParam String email, HttpSession session, HttpServletRequest request)
		...
		if (ObjectUtils.isEmpty(userByEmail)) {
			...
			if (sendMail) {
				session.setAttribute("SuccessMsg", "Please check your mail inbox for password reset link.");
			} else {
				session.setAttribute("ErrorMsg", "Something went wrong!");
			}
		}
